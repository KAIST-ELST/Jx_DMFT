...
function input_handler(DMFT_Jx_option)
   ...
   if (DMFT_Jx_option.Calculation_mode == "DMFT")
    ...
    elseif (DMFT_Jx_option.Calculation_mode == "Jx0" || DMFT_Jx_option.Calculation_mode == "Magnon")
        Calculation_mode = DMFT_Jx_option.Calculation_mode
        println(" Calculation mode :", Calculation_mode)
...
elseif (arg_input.Optional["DMFT_Jx"].Calculation_mode == "Jx0" || arg_input.Optional["DMFT_Jx"].Calculation_mode == "Magnon")
...
function band_kpts2(kpath,totgrid)
    ...
    for i =2:size(kpath)[1]
        ...
        dumleng += length_
        push!(xleng,dumleng)
        totlength += ((kpath[i][1]-kpath[i-1][1])^2+(kpath[i][2]-kpath[i-1][2])^2+(kpath[i][3]-kpath[i-1][3])^2)^(1/2)
    end
    xpt = range(0, totlength, length=totgrid)
    xpt_group = zeros(Int64,size(xpt)[1])
    for i = 1:size(xpt)[1]
        for j =2:size(kpath)[1]
            if(0.0<xpt[i]<=xleng[1])
                xpt_group[i]=1
            elseif(xleng[j-1]<xpt[i]<=xleng[j])
                xpt_group[i]=j
            end
        end
    end
    ...
    for i=2:size(kpath)[1]
        ind_list = findall(x->x==i-1, xpt_group)
     
        grid = size(ind_list)[1]
        k1=range(kpath[i-1][1], stop=kpath[i][1], length =grid+1)
        k2=range(kpath[i-1][2], stop=kpath[i][2], length =grid+1)
        k3=range(kpath[i-1][3], stop=kpath[i][3], length =grid+1)
...
function Cal_MagMat(qpoint, rel_pos_frac, R_grid_num, cellvec, Corr_atom_Ind, Corr_atom_equiv, Moment, Neigh_upperbound, ordered_dist, dist, Jmat)
    Orbind_num = [size(Jmat)[4],  size(Jmat)[5]]
    Magmat_ = zeros(ComplexF64, size(qpoint)[1], Orbind_num[1], Orbind_num[2])
    considered_Neigh = zeros(Int64, size(Corr_atom_Ind)[1], Neigh_upperbound)
  
    sign = Corr_atom_equiv./(abs.(Corr_atom_equiv))

    Magnon_disp = zeros(ComplexF64, size(qpoint)[1], Orbind_num[1])
    
    for qind =1:size(qpoint)[1]
        for (o1,o1val) in enumerate(Corr_atom_Ind)
            for (o2,o2val) in enumerate(Corr_atom_Ind)
                for R1 = 1:R_grid_num[1]
                    for R2 = 1:R_grid_num[2]
                        for R3 = 1:R_grid_num[3]
                            #Magmat_[qind, o1,o1] += (4.0/Moment[o1])*(Jmat[R1,R2,R3,o1,o2]) 
                            #Magmat_[qind, o1,o2] -= (4.0/Moment[o1])*(Jmat[R1,R2,R3,o1,o2])*exp(im* dot(qpoint[qind,:], cellvec[R1,R2,R3,:].-rel_pos_frac[o1val,o2val,:]) )
    
                            if ( dist[R1,R2,R3,o1,o2] <= ordered_dist[o1][Neigh_upperbound+1]+0.00001 )
                                for n = 1:Neigh_upperbound
                                ...
                                #Magmat_[qind, o1,o2] -= (4.0/Moment[o1])*(Jmat[R1,R2,R3,o1,o2])*sign[o1]*exp( 2*pi*im* dot(qpoint[qind,:], cellvec[R1,R2,R3,:].-rel_pos_frac[o1val,o2val,:] ) )
                            
                                #Magmat_[qind, o1,o1] += (4.0/Moment[o1])*(Jmat[R1,R2,R3,o1,o2])*sign[o2]  
                                #Magmat_[qind, o1,o2] -= (4.0/Moment[o1])*(Jmat[R1,R2,R3,o1,o2])*sign[o1]*exp( 2*pi*im* dot(qpoint[qind,:], [R1-1, R2-1, R3-1]) )
                                #Magmat_[qind, o1,o2] -= (4.0/Moment[o1])*(Jmat[R1,R2,R3,o1,o2])*sign[o1]*exp( 2*pi*im* dot(qpoint[qind,:], [R1-1, R2-1, R3-1].-rel_pos_frac[o1val,o2val,:] ) )
...
if ( Calculation_mode == "Magnon" )
    ...
    println("")
    Neigh_upperbound = 2   # For magnon-dispersion, the magnetic interactions are consider by <=Neigh_upperbound 
    Moment = [1.68, 1.68]  #magnetic moment size, unit :Bohr magneton
    println("Magnetic moment size :", Moment, " (unit : Bohr magneton)")
    println("")
    ...
    kpoint, xpt,lattind= band_kpts2(kpath,tot_grid)
    
     
    considered_Neigh, Magmat, Magnon_disp =Cal_MagMat(kpoint, rel_pos_frac, R_grid_num, cellvec, Corr_atom_Ind, Corr_atom_equiv, Moment, Neigh_upperbound, ordered_dist, dist, Jmat)
    println()
    
    println("Considered Neighbors : ")
    for i=1:size(considered_Neigh)[1]
        println("  # of 1st NN, 2nd NN ... :", considered_Neigh[i,:])
    end
    println()
